...
BANNER
    ...
    module GitHubApiHelper
      ...
      def get_files_in_master
        master_tree_sha = open('https://api.github.com/repos/huluren/kaminari_themes/git/refs/heads/master') do |json|
          ActiveSupport::JSON.decode(json.read)['object']['sha']
        end
        open('https://api.github.com/repos/huluren/kaminari_themes/git/trees/' + master_tree_sha + '?recursive=1') do |json|
      ...
      def get_content_for(path)
        open('https://api.github.com/repos/huluren/kaminari_themes/contents/' + path) do |json|
